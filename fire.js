...
function renderFire(){
    const debug = false;
    let ht = '<table cellpadding=0 cellspacing=0>';
    ...
    for (let row = 0; row < fireHeight ; row++){
        ...
        for (let col = 0; col < fireWidth; col++){
            ...
            if(debug == true){
                ht += '<td>';
                ht += `<div class="pixel-index">${pixelIndex}</div>`;
                ht += fireIntensity;
                ht += '</td>';
            }else{
                const color = fireColorPalette[fireIntensity];
                const colorString = `${color.r},${color.g},${color.b}`;
                ht += `<td class="pixel" style="background-color: rgb(${colorString})">`;
                ht += '</td>';
            }
    ...
    document.getElementById('fireCanvas').innerHTML = ht;
}
